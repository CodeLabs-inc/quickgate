...
function TransitLists() {
    ...
    const fetchTransits = async (page: number) => {
        setLoading(true);
        if (containerRef.current) {
            console.log('Setting scroll position to:', containerRef.current.scrollTop);
            setScrollPosition(containerRef.current.scrollTop);
        }
        const call = await getTransits(page, limit, search, filter);
        if (call.success) {
    ...
    const onSearch = async (value: string, filter: string) => {
        setLoading(true);
        const call = await getTransits(page, limit, value, filter);
        if (call.success) {
            setGates(call.data);
        } else {
            toast.error(call.message);
            ...
            >
                {!loading && leads.length > 0 ? (
                    leads.map((lead: any, index: number) => (
                        <TableCard
                            key={index}
                            main={lead.licensePlate}
                            others={[
                                new Date(lead.dates.entry).toLocaleDateString('it-IT', { month: 'numeric', day: 'numeric', hour: 'numeric', minute: 'numeric' }),
                                lead.dates.exit ? new Date(lead.dates.exit).toLocaleDateString('it-IT', { month: 'numeric', day: 'numeric', hour: 'numeric', minute: 'numeric' }) : 'En curso',
                                lead.value ? `${formatThousandsSeparators(lead.value)} $RD` : '--'
                            ]}
                        />
                    ))
                ) : !loading && leads.length === 0 ? (
