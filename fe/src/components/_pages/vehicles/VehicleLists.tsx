...
import { getAllVehicles, getTransits} from '@/services/api';
...
function VehicleLists() {
    const containerRef = React.useRef<HTMLDivElement>(null);
    const [loading, setLoading] = React.useState(true);
    const [leads, setVehicles] = React.useState<any>([]);
    ...
    const fetchVehicles = async (page: number) => {
        ...
        const call = await getAllVehicles(page, limit, search, filter);
        if (call.success) {
            console.log(call.data)
            setVehicles((prev: any) => [...prev, ...call.data]);
        } else {
    ...
    const onSearch = async (value: string, filter: string) => {
        setLoading(true);
        const call = await getAllVehicles(page, limit, value, filter);
        if (call.success) {
